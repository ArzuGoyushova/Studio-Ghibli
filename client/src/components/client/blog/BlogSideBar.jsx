...
const BlogSidebar = ({blogs, onSearch, onCategoryClick}) => {
    ...
    const fetchCategories = async () => {
        try {
            ...
            const filteredCategories = response.data.filter(category => category.parentId === "8ab24a65-e725-4c00-d19c-08db90132238");
    ...
    const shuffleBlogs = () => {
        ...
        const choosenBlogs = shuffledBlogs.slice(0, 3);
        setRandomBlogs(choosenBlogs);
    };
            ...
            <div className={`${menuVisible || 'hidden md:block'}`}>
                <div className="mx-2 my-5 py-5 border-b border-t">
                    ...
                    {categories.map(category => (
                            ...
                            >
                                <a className="text-linkColor p-2 hover:bg-red-200 hover:rounded-xl">{category.name}</a>
                            </li>
                        ))}
                    </ul>
                </div>
                <div className="mx-2 my-5 py-5 border-b">
                    <h3 className="font-semibold mb-3 text-xl">Blogs just for You</h3>
                    <ul className="space-y-1">
                        {randomBlogs.map(blog => (
                            <li className='text-sm flex items-center' key={blog.id}>
                                <img className="w-1/5 me-2" src={`./images/blog/${blog.imageUrl}`}/>
                                <Link className="text-linkColor cursor-pointer p-2 hover:bg-red-200 hover:rounded-xl" to={`/blog/${blog.id}`}>{blog.title}</Link>
                            </li>
                        ))}
                    </ul>
                </div>
